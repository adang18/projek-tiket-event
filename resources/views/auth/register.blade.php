...
<div class="bg-white p-6 rounded shadow-md w-full max-w-md">
    ...
    <form action="{{ route('register') }}" method="POST" class="space-y-4">
        @csrf
        <input type="text" name="name" placeholder="Nama" class="w-full p-2 border rounded" value="{{ old('name') }}" required>
        ...
        <button type="submit" class="bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700 transition">Daftar</button>
    </form>

    <p class="mt-4 text-gray-600">Sudah punya akun? <a href="{{ route('login.form') }}" class="text-blue-600 hover:underline">Login di sini</a></p>
</div>
</body>
</html>
